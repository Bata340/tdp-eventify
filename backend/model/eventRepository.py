...
class EventRepository:
        ...
        if email_request is not None:
            for event in events:
                if event["owner"] == email_request:
                    ...
                    favEvent = self.favouriteEvents["favouriteEvents"].find_one(
                        {"user_email": email_request, "event_id": event["_id"]["$oid"]})
    ...
    def deleteEventWithId(self, id: str):
        deleted_event = self.database["events"].delete_one(
            {"_id": ObjectId(id)})
    ...
    def create_reservation(self, reservation: dict):
        new_event_reservation = self.reserveEvents["reservedEvents"].insert_one(
            reservation)
        event_reservation_created = self.reserveEvents["reservedEvents"].find_one(
            {"_id": new_event_reservation.inserted_id})

        return json.loads(json_util.dumps(event_reservation_created))

    def getEventsFromUser(self, userId: str):
        filter = {'userid': userId}
        reservedEvents = self.reserveEvents["reservedEvents"].find(
            filter=filter)
    ...
    def getTransactionsFromUser(self, userId: str):
        filter = {'userId': userId}
        returnedTransactions = self.transactions["transactions"].find(
            filter=filter)
    ...
    def createTransaction(self, tx: dict):
        ...
        newTransaction = self.transactions["transactions"].find_one(
            {"_id": transaction.inserted_id})
        return json.loads(json_util.dumps(newTransaction))

    def toggleFavourite(self, fields: dict):
        favouriteDocs = self.favouriteEvents["favouriteEvents"].find_one(
            fields)
        if favouriteDocs is not None:
            self.favouriteEvents["favouriteEvents"].delete_one(
                {"_id": favouriteDocs["_id"]})
    ...
    def getFavouriteEvents(self, user_email: str):
        favEvents = self.favouriteEvents["favouriteEvents"].find(
    ...
    def getEventAtendees(self, event_id: str):
        atendees = self.reserveEvents["reservedEvents"].find(
            {"event_id": event_id})
        if atendees is None:
            return []
        emails = []
        for user in list(json.loads(json_util.dumps(atendees))):
            emails.append(user['userid'])
        return emails
